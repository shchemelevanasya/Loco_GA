...
Изменения в этой версии:
 ...
 - Можно выбрать подмножество операторов кроссовера или использовать все одновременно; адаптация ведётся
   только внутри выбранного набора операторов.
"""
...
def generate_synthetic_data(
    ...
    """
    Генерация синтети��еских данных.
...
def fitness_function_components_based(chromosome: Chromosome,
    ...
    """
    Вычисляет fitness на основе 5 компоне��тов. Если weights не заданы, используются равные веса.
    """
...
class GeneticAlgorithm:
    ...
    def _adapt_mutation_rate(self, current_best: float):
        ...
        Простая адаптивная логика:
         - если best улучшился (на eps) — уменьшаем mutation_rate (исследование не ��ребуется так сильно)
    ...
    def _generate_children(self, population: List[Chromosome], target_count: int) -> List[Chromosome]:
        ...
        if self.use_multiprocessing and len(population) >= self.multiprocessing_threshold and self.cpu_count > 1:
            ...
            for _ in range(target_count * 3):
                ...
                # sample crossover method according to adaptive weights (only among allowed)
