...
if not logger.handlers:
    ...
    try:
        ...
        # running in environments where file logging may be disabled
        pass
...
class Train:
    ...
    duration: float
    category: int = 0  # optional priority category (0 = low)
...
class GeneticAlgorithm:
    ...
    # selection helpers that respect maximize/minimize
...
def run_streamlit_app():
    ...
    st.sidebar.markdown("Mutations (select)")
    m_swap = st.sidebar.checkbox("swap_locos", value=True)
    m_replace = st.sidebar.checkbox("replace_loco", value=True)
    m_range = st.sidebar.checkbox("range_shuffle", value=True)
    mutation_methods = []
    if m_swap:
    ...
    adaptive_mutation = st.sidebar.checkbox("Adaptive mutation rate", value=True)
    min_mut = st.sidebar.slider("Min mutation (adaptive)", 0.0, 1.0, 0.01, 0.01)
    max_mut = st.sidebar.slider("Max mutation (adaptive)", 0.0, 1.0, 0.5, 0.01)

    st.sidebar.markdown("Optimization direction")
    maximize = st.sidebar.checkbox("Maximize (unchecked = Minimize)", value=False)

    st.sidebar.markdown("Multiprocessing")
    use_mp = st.sidebar.checkbox("Use multiprocessing", value=True)
    mp_threshold = st.sidebar.number_input("Multiprocessing threshold (population size)", min_value=2, max_value=500, value=100)
    ...
    if use_uploaded and loco_file is not None:
        try:
            ...
            st.session_state["locomotives"] = locomotives
            st.success("Locomotives loaded")
        except Exception as e:
            st.error(f"Failed to parse locomotives file: {e}")

    if use_uploaded and trains_file is not None:
        try:
            trains = parse_trains_csv(trains_file)
            st.session_state["trains"] = trains
            st.success("Trains loaded")
        except Exception as e:
            st.error(f"Failed to parse trains file: {e}")
    ...
    with right:
        ...
        st.markdown("### Crossover weights")
        crossover_weights_placeholder = st.empty()
        st.markdown("### Time & result")
    ...
    with left:
        ...
        st.markdown("### Components evolution")
    ...
    if run_button:
        def progress_callback(gen: int, stats: Dict[str, Any]):
            ...
            gen_stats_placeholder.metric(label=f"Generation {gen}", value=f"Best: {best_signed}", delta=f"Mean: {mean:.6f}")
        ...
        try:
            ...
            logger.exception("GA run error: %s", e)
    ...
    if "solution" in st.session_state:
        ...
        df_assign = pd.DataFrame() if solution is None else build_assignment_dataframe(solution, locomotives, trains)
        assignment_placeholder.dataframe(df_assign, height=350)
        st_download_button_from_df(df_assign, "assignment table", filename="assignment.csv")

        df_loco = build_locomotive_summary_dataframe(locomotives, trains, solution)
        loco_summary_placeholder.table(df_loco)
        st_download_button_from_df(df_loco, "locomotive summary", filename="loc_summary.csv")

        fig_timeline = plot_assignment_matplotlib(solution, trains)
        timeline_placeholder.pyplot(fig_timeline)
        st_download_button_from_fig(fig_timeline, "assignment timeline", filename="assignment_timeline.png", fmt="png", dpi=300)

        fig_gc = plot_generation_curve(reporter)
        evolution_placeholder.pyplot(fig_gc)
        st_download_button_from_fig(fig_gc, "generation curve", filename="generation_curve.png", fmt="png", dpi=300)

        fig_w = plot_weights_evolution(reporter)
        weights_plot_placeholder.pyplot(fig_w)
        st_download_button_from_fig(fig_w, "weights evolution", filename="weights_evolution.png", fmt="png", dpi=300)

        fig_cw = plot_crossover_weights_evolution(reporter)
        crossover_weights_placeholder.pyplot(fig_cw)
        st_download_button_from_fig(fig_cw, "crossover weights", filename="crossover_weights.png", fmt="png", dpi=300)

        fig_c = plot_components_evolution(reporter)
        components_placeholder.pyplot(fig_c)
        st_download_button_from_fig(fig_c, "components evolution", filename="components_evolution.png", fmt="png", dpi=300)

        total_time = st.session_state.get("total_time", None)
        time_summary_placeholder.markdown(f"**Total computation time:** {total_time:.4f} s")
        time_summary_placeholder.markdown(f"**Final objective (raw):** {solution.fitness:.6f}")

    if st.sidebar.button("Export generation stats CSV") and "reporter" in st.session_state:
        rep: GAReporter = st.session_state["reporter"]
        df = pd.DataFrame(rep.generation_log)
        st_download_button_from_df(df, "generation stats", filename="generation_stats.csv")
        st.success("Exported")
...
if __name__ == "__main__":
    if STREAMLIT_AVAILABLE and os.environ.get("RUN_STREAMLIT", "1") == "1":
        ...
        logger.info("Streamlit unavailable or disabled — running CLI demo")
        run_cli_demo()