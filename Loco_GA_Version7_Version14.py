...
Полный файл с исправлениями:
 - Вставлена функция generate_initial_population (устраняет NameError).
 - Визуализация и UI на русском языке.
 - Поддержка выбора операторов кроссовера, per-operator probabilities, адаптивная мутация.
 - Поддержка минимизации и максимизации (опция в UI).
 - Надёжный fallback оформления matplotlib при отсутствии seaborn.
 - Улучшённые графики для исследовательских отчётов и экспорт PNG/CSV.
"""
...
# Попытка импортировать seaborn; если нет — подобрать безопасный стиль matplotlib
try:
...
# Глобальная настройка Matplotlib для качественных графиков
plt.rcParams.update({
    "figure.dpi": 150,
    "savefig.dpi": 300,
    "font.size": 10,
...
if not logger.handlers:
    ...
    try:
        ...
        # В окружениях без доступа к файловой системе FileHandler может упасть; игнорируем
        pass
...
# ---------------------
# Утилиты для экспорта фигур/таблиц
# ---------------------
...
# ---------------------
# Геометрия и перевод расстояния-время
# ---------------------
...
# ---------------------
# Хромосома и проверка реализуемости
# ---------------------
class Chromosome:
    ...
    for loco_id, train_ids in chrom.assignment.items():
        ...
        for t in sorted_trains:
            ...
            if loco_arrival > t.departure_time:
                train_wait_time_sum += (loco_arrival - t.departure_time)
            else:
                loco_wait_time_sum += (t.departure_time - loco_arrival)
...
# ---------------------
# generate_initial_population — вставлена для устранения NameError
# ---------------------
def generate_initial_population(population_size: int,
    ...
    """
    Генерация начальной популяции корректных по быстрой проверке хромосом.
    Если не удаётся найти достаточное количество, создаются вариации детерминированного распределения.
    """
    population: List[Chromosome] = []
    if not locomotives:
        raise ValueError("Не переданы локомотивы для генерации начальной популяции")
    ...
    if len(population) < population_size:
        # fallback deterministic base
        base_assignment = {lid: [] for lid in loco_ids}
        for i, t in enumerate(train_ids_master):
            base_assignment[loco_ids[i % len(loco_ids)]].append(t)
        base_chrom = Chromosome(copy.deepcopy(base_assignment))
        if is_feasible_fast(base_chrom, locomotives, trains):
            population.append(base_chrom)
        else:
            population.append(base_chrom)
        i = 0
        while len(population) < population_size and attempts < max_attempts * 5:
            cand = copy.deepcopy(base_assignment)
            if len(loco_ids) >= 2 and random.random() < 0.6:
                a, b = random.sample(loco_ids, 2)
                cand[a], cand[b] = cand[b], cand[a]
            else:
                src = random.choice(loco_ids)
                if cand[src]:
                    t = random.choice(cand[src])
                    cand[src].remove(t)
                    dst = random.choice([l for l in loco_ids if l != src])
                    cand[dst].append(t)
            cand_chrom = Chromosome(cand)
            if is_feasible_fast(cand_chrom, locomotives, trains):
                population.append(cand_chrom)
            else:
                # если не хватает, допускаем добавление и продолжаем
                population.append(cand_chrom)
            i += 1
            attempts += 1

    if not population:
        # крайний случай — всё на первом локомотиве
        assignment = {lid: [] for lid in loco_ids}
        for t in train_ids_master:
            assignment[loco_ids[0]].append(t)
        population.append(Chromosome(assignment))

    # привести к требуемому размеру
    if len(population) > population_size:
        population = population[:population_size]
    else:
        idx = 0
        while len(population) < population_size:
            population.append(Chromosome(copy.deepcopy(population[idx % len(population)].assignment)))
            idx += 1
...
# ---------------------
# Помощники: представления и операторы
# ---------------------
...
# ---------------------
# GeneticAlgorithm (как выше) — уже использует generate_initial_population
# ---------------------
class GeneticAlgorithm:
    ...
    def _tournament_selection(self, population: List[Chromosome], k: int = 3) -> Chromosome:
        k = min(k, len(population))
        candidates = random.sample(population, k)
        return max(candidates, key=lambda c: c.fitness) if self.maximize else min(candidates, key=lambda c: c.fitness)
    ...
    def _evaluate_population_dynamic(self, population: List[Chromosome]) -> Dict[str, Any]:
        ...
        best = max(fitnesses) if (fitnesses and self.maximize) else (min(fitnesses) if fitnesses else (float("inf") if not self.maximize else float("-inf")))
...
# ---------------------
# Репортер и улучшённые графики (на русском)
# ---------------------
class GAReporter:
    def __init__(self):
...
# ---------------------
# Функции построения таблиц и графиков (русские подписи)
# ---------------------
...
def plot_generation_curve(reporter: GAReporter, figsize: Tuple[int, int] = (10, 4), dpi: int = 150) -> plt.Figure:
    ...
    if not reporter.generation_log:
        ax.set_title("Кривая эволюции (данные отсутствуют)")
        return fig
    ...
    std = df["std"].to_numpy()
    ax.plot(gens, best, label="Лучшее (raw)", color="#1f77b4", marker="o", linewidth=1.5)
    ax.plot(gens, mean, label="Среднее", color="#ff7f0e", marker="s", linewidth=1.2)
...
def plot_weights_evolution(reporter: GAReporter, figsize: Tuple[int, int] = (10, 3), dpi: int = 150) -> plt.Figure:
    fig, ax = plt.subplots(figsize=figsize, dpi=dpi)
    if not reporter.generation_log:
        ax.set_title("Динамика весов (данные отсутствуют)")
        return fig
    ...
    ax.set_xlabel("Поколение")
    ax.set_ylabel("Нормированные веса")
    ax.set_title("Эволюция весов критериев")
    ax.grid(True, linestyle="--", alpha=0.5)
    ax.legend(loc="best")
    return fig
...
def plot_crossover_weights_evolution(reporter: GAReporter, figsize: Tuple[int, int] = (10, 3), dpi: int = 150) -> plt.Figure:
    fig, ax = plt.subplots(figsize=figsize, dpi=dpi)
    if not reporter.generation_log:
        ax.set_title("Динамика весов операторов кроссовера (данные отсутствуют)")
        return fig
    ...
    ax.set_xlabel("Поколение")
    ax.set_ylabel("Адаптивный вес")
    ax.set_title("Динамика весов операторов кроссовера")
    ax.grid(True, linestyle="--", alpha=0.5)
    ax.legend(loc="best")
    return fig
...
def plot_components_evolution(reporter: GAReporter, figsize: Tuple[int, int] = (10, 4), dpi: int = 150) -> plt.Figure:
    fig, ax = plt.subplots(figsize=figsize, dpi=dpi)
    if not reporter.generation_log:
        ax.set_title("Динамика компонент (данные отсутствуют)")
        return fig
    gens = [e["поколение"] for e in reporter.generation_log]
    ...
    ax.set_xlabel("Поколение")
    ax.set_ylabel("Средние значения компонент")
    ax.set_title("Динамика компонент цели по поколениям")
    ax.grid(True, linestyle="--", alpha=0.5)
    ax.legend(loc="best")
    return fig
...
def plot_assignment_matplotlib(solution: Chromosome, trains: Dict[int, Train],
    ...
    for loco_id, train_ids in loco_items:
        for t_id in sorted(train_ids, key=lambda tid: trains[tid].departure_time):
            ...
            ax.text(t.departure_time + t.duration / 2, y, f"{t.id} (кат{t.category})", va="center", ha="center", color="white", fontsize=7)
...
def run_streamlit_app():
    if not STREAMLIT_AVAILABLE:
        print("Streamlit не установлен. Установите streamlit для запуска UI.")
        return
    ...
    use_uploaded = st.sidebar.checkbox("Использовать загруженные файлы", value=True)
    ...
    mutation_rate = st.sidebar.slider("Начальная вероятность мутации", 0.0, 1.0, 0.15, 0.01)
    ...
    if use_all_cross:
        ...
        if not crossover_methods_selected:
            st.sidebar.warning("Не выбран ни один оператор — будет использован 'uniform'")
            crossover_methods_selected = ["uniform"]

    st.sidebar.markdown("Задайте вероятность применения для каждого выбранного оператора")
    ...
    st.sidebar.markdown("Мутации (выберите применимые):")
    m_swap = st.sidebar.checkbox("swap_locos (обмен назначениями)", value=True)
    m_replace = st.sidebar.checkbox("replace_loco (перенос поезда)", value=True)
    m_range = st.sidebar.checkbox("range_shuffle (перемешивание диапазона)", value=True)
    mutation_methods = []
    if m_swap:
    ...
    adaptive_mutation = st.sidebar.checkbox("Адаптивная вероятность мутации", value=True)
    ...
    maximize = st.sidebar.checkbox("Maximize (если снято — Minimize)", value=False)

    st.sidebar.markdown("Multiprocessing")
    use_mp = st.sidebar.checkbox("Использовать multiprocessing", value=True)
    mp_threshold = st.sidebar.number_input("Порог для multiprocessing (размер популяции)", min_value=2, max_value=500, value=100)

    if st.sidebar.button("Сгенерировать синтетические данные"):
        with st.spinner("Генерируются данные..."):
    ...
    with left:
        ...
        st.markdown("### Динамика компонент")
    ...
    if run_button:
        ...
        try:
            ...
            with st.spinner("Выполняется GA..."):
                t0 = time.time()
                best = ga.run()
    ...
    if "solution" in st.session_state:
        ...
        st_download_button_from_df(df_assign, "таблица назначений", filename="assignment.csv")

        df_loco = build_locomotive_summary_dataframe(locomotives, trains, solution)
        loco_summary_placeholder.table(df_loco)
        st_download_button_from_df(df_loco, "сводная таблица локомотивов", filename="locomotives_summary.csv")

        fig_timeline = plot_assignment_matplotlib(solution, trains)
        timeline_placeholder.pyplot(fig_timeline)
        st_download_button_from_fig(fig_timeline, "временная диаграмма назначений", filename="assignment_timeline.png", fmt="png", dpi=300)

        fig_gc = plot_generation_curve(reporter)
        evolution_placeholder.pyplot(fig_gc)
        st_download_button_from_fig(fig_gc, "кривая эволюции", filename="generation_curve.png", fmt="png", dpi=300)

        fig_w = plot_weights_evolution(reporter)
        weights_plot_placeholder.pyplot(fig_w)
        st_download_button_from_fig(fig_w, "динамика весов", filename="weights_evolution.png", fmt="png", dpi=300)

        fig_cw = plot_crossover_weights_evolution(reporter)
        crossover_weights_placeholder.pyplot(fig_cw)
        st_download_button_from_fig(fig_cw, "веса кроссоверов", filename="crossover_weights.png", fmt="png", dpi=300)

        fig_c = plot_components_evolution(reporter)
        components_placeholder.pyplot(fig_c)
        st_download_button_from_fig(fig_c, "компоненты", filename="components_evolution.png", fmt="png", dpi=300)

        total_time = st.session_state.get("total_time", None)
        time_summary_placeholder.markdown(f"**Общее время расчёта:** {total_time:.4f} с")
        time_summary_placeholder.markdown(f"**Итоговая целевая функция (raw):** {solution.fitness:.6f}")

    if st.sidebar.button("Экспортировать статистику поколений (CSV)") and "reporter" in st.session_state:
        rep: GAReporter = st.session_state["reporter"]
        df = pd.DataFrame(rep.generation_log)
        st_download_button_from_df(df, "статистика поколений", filename="generation_stats.csv")
...
def run_cli_demo():
    ...
    df = build_assignment_dataframe(best, locomotives, trains)
    print(df)
...
if __name__ == "__main__":
    if STREAMLIT_AVAILABLE and os.environ.get("RUN_STREAMLIT", "1") == "1":
        ...
        logger.info("Streamlit недоступен или отключён — запускаю CLI демо")
        run_cli_demo()