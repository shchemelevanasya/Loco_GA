...
Изменения в этой версии:
 - Добавлены новые операторы мутаций (swap, replace, range-shuffle) и новые операторы кроссовера
   (one-point, two-point, uniform, priority-by-train-category).
 - Добавлена адаптивная скорость мутации (адаптируется по истории лучших значений fitness).
 - Streamlit UI расширен: окно загрузки данных (локомотивы / поезда) и выбор операторов кроссовера/мутации.
"""
...
def mutation_assignment(assignment: Dict[int, List[int]],
    ...
    mutation_rate — базовая вероятность применения операции на отдельном элементе, но дл�� некоторых
        ...
        if self.use_multiprocessing and len(population) >= self.multiprocessing_threshold and self.cpu_count > 1:
            ...
            for _ in range(target_count * 3):
                ...
                parent_pairs.append((p1.assignment, p2.assignment, self.locomotives, self.trains,
                                     self.crossover_method, curr_mutation_rate, self.mutation_methods))
            ...
            while len(children) < target_count and attempts < target_count * 50:
                ...
                child_assign = crossover_assignments(p1.assignment, p2.assignment, self.trains, method=self.crossover_method)
...
def run_streamlit_app():
    ...
    crossover_method = st.sidebar.selectbox("Кроссовер", options=["uniform", "one_point", "two_point", "priority"], index=0)
    ...
    m_range = st.sidebar.checkbox("range_shuffle (перемешивание по диап��зону локомотивов)", value=True)
    mutation_methods = []
    if m_swap:
