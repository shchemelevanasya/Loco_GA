...
Изменения в этой версии:
 ...
 - Можно выбрать подмножество операторов кроссовера или использовать все одновременно; адаптация ведётся
   только внутри выбранного набора операторов. Выбор операторов кроссовера теперь аналогичен выбору
   операторов мутации: можно включить несколько операторов и каждый оператор применится с некоторой
   вероятностью (crossover_apply_rate), при этом адаптивные веса влияют на вероятность применения.
"""
...
# ---------------------
# Population initialization (RESTORED)
# ---------------------
def generate_initial_population(population_size: int,
    ...
    """
    Генерирует начальную популяцию в виде корректных (по быстрой проверке) хромосом.
    Использует случайное распределение поездов по локомотивам с повторными попытками.
    Если не удаётся найти допустимую популяцию — создаётся простая детерминированная хромосома.
    """
    ...
    if not population:
        # fallback deterministic assignment to ensure at least one individual
        assignment = {lid: [] for lid in loco_ids}
...
# ---------------------
# Multiprocessing workers (updated to support multi-operator crossover)
# ---------------------
...
def _child_worker_serial(args):
    """
    args:
      assign_p1, assign_p2, locomotives, trains, mutation_rate,
      crossover_apply_rate, crossover_methods, crossover_weights, mutation_methods
    """
    (assign_p1, assign_p2, locomotives, trains,
     mutation_rate, crossover_apply_rate, crossover_methods, crossover_weights, mutation_methods) = args

    # choose base parent randomly
    ...
    child_assign = base
    # avg weight among active methods
    if crossover_methods and crossover_weights:
        avg = 1.0 / len(crossover_methods)
    else:
        avg = 1.0

    for i, method in enumerate(crossover_methods):
        weight = crossover_weights[i] if (crossover_weights and i < len(crossover_weights)) else avg
        # compute per-method probability: base apply rate scaled by relative weight
        prob = crossover_apply_rate * max(0.0, 1.0 + (weight - avg))
        if random.random() < prob:
            # apply operator between current child and the other parent
            try:
                child_assign = crossover_assignments(child_assign, other, trains, method=method)
            except Exception:
                # fallback: try to apply using p1/p2 directly
...
# ---------------------
# GeneticAlgorithm with adaptive crossover selection (crossover analogous to mutation)
# ---------------------
class GeneticAlgorithm:
    def __init__(self, locomotives: Dict[int, Locomotive], trains: Dict[int, Train],
                 ...
                 crossover_apply_rate: float = 0.5):
        ...
        self.crossover_apply_rate = float(crossover_apply_rate)

        # adaptive tracking
        self.best_history: List[float] = []
        self.no_improve_generations = 0

        # All possible crossover operators
        all_ops = ["one_point", "two_point", "uniform", "priority"]
        # Determine which crossover methods to use: either provided subset or all
        if crossover_methods_allowed:
            # keep specified operators but only those known
            self.crossover_methods = [m for m in all_ops if m in crossover_methods_allowed]
            if not self.crossover_methods:
                # fallback to all if user selection was invalid/empty
                self.crossover_methods = all_ops[:]
        else:
            self.crossover_methods = all_ops[:]

        # initialize adaptive weights uniformly across chosen methods
    ...
    def _adapt_mutation_rate(self, current_best: float):
        ...
        Простая адаптивная логика:
         - если best улучшился (на eps) — уменьшаем mutation_rate (исследование не требуется так сильно)
    ...
    def _adapt_crossover_weights(self, current_best: float):
        """
        Адаптируем вес�� операторов кроссовера на основе улучшения best:
        - при улучшении: повышаем вероятность эксплуатационных операторов (one_point, priority)
        - при стагнации: повышаем вероятность исследовательских операторов (uniform, two_point)
        Поддерживаем нормировку и ограничения. Адаптация проводится только среди self.crossover_methods.
        """
        ...
        min_w = 0.01
        # adjust only methods that exist in self.crossover_methods
        ...
        if tot <= 0:
            # reset to uniform over currently active methods
            self.crossover_weights = [1.0 / len(self.crossover_methods) for _ in self.crossover_methods]
        else:
        ...
        if self.use_multiprocessing and len(population) >= self.multiprocessing_threshold and self.cpu_count > 1:
            ...
            for _ in range(target_count * 3):
                ...
                parent_pairs.append((
                    ...
                    curr_mutation_rate, self.crossover_apply_rate, self.crossover_methods,
            ...
            with concurrent.futures.ProcessPoolExecutor(max_workers=min(self.cpu_count, len(parent_pairs))) as exc:
                for result in exc.map(_child_worker_serial, parent_pairs):
                    if result is not None:
                        ...
                        # best-effort increment usage counts: worker doesn't return usage breakdown,
                        # so approximate by adding 1 to a random method that exists
            ...
            while len(children) < target_count and attempts < target_count * 50:
                ...
                for i, method in enumerate(self.crossover_methods):
                    ...
                    prob = self.crossover_apply_rate * max(0.0, 1.0 + (weight - avg))
                    if random.random() < prob:
                        # apply operator to current child and the other parent
                        try:
    ...
    def run(self) -> Chromosome:
        ...
        for gen in range(self.generations):
            ...
            # adapt crossover weights based on improvement/stagnation (only among chosen methods)
            self._adapt_crossover_weights(stats["best"])

            # record current crossover weights in stats (matching order of self.crossover_methods)
            ...
            stats["crossover_apply_rate"] = round(self.crossover_apply_rate, 4)
...
def plot_crossover_weights_evolution(reporter: GAReporter):
    if not reporter.generation_log:
        ...
        return fig
    gens = [e["поколение"] for e in reporter.generation_log]
    # each entry may have different active methods; prepare series per known method
    methods_all = ["one_point", "two_point", "uniform", "priority"]
    # build per-method weight series, fill missing with 0
...
def run_streamlit_app():
    ...
    st.sidebar.markdown("Вероятность применения каждого выбранного оператора кроссовера (аналогично мутации)")
    crossover_apply_rate = st.sidebar.slider("crossover_apply_rate", 0.0, 1.0, 0.5, 0.05)
    ...
    if run_button:
        ...
        try:
            ga = GeneticAlgorithm(
                ...
                crossover_apply_rate=float(crossover_apply_rate)
            )
