...
Изменения в этой версии:
 ...
 - Адаптивная скорость мутации сохранена.
 - Можно выбирать подмножество операторов кроссовера и задавать индивидуальную вероятность применения
   для каждого оператора (аналогично мутациям). Адапивная логика меняет относительные веса операторов,
   а итоговая вероятность применения конкретного оператора = base_prob_operator * scaling_from_adaptive_weight.
 - Streamlit UI обновлён: выбор операторов кроссовера + слайдеры вероятностей для каждого выбранного оператора.
"""
...
def compute_time_components_for_chromosome(chrom: Chromosome,
    ...
    Для одной хромосомы считаем:
      - idle_time_sum: суммарное время, когда локомотив простаивал в ожидании ре��са (локомотив раньше прибыл)
...
# ---------------------
# Multiprocessing workers (updated to support per-operator probs)
# ---------------------
...
def _child_worker_serial(args):
    ...
    args:
      ...
      mutation_rate, crossover_operator_probs (dict),
      crossover_methods (list), crossover_weights (list), mutation_methods (list)
    """
    (assign_p1, assign_p2, locomotives, trains,
    ...
    # average weight for scaling reference
    ...
    for i, method in enumerate(crossover_methods):
        ...
        # scale base probability by relative adaptive weight (simple linear scaling)
        # scaling factor = 1 + (weight - avg_weight)
        ...
        if random.random() < prob:
            try:
                ...
                # fallback: apply operator between assign_p1 and assign_p2
                child_assign = crossover_assignments(assign_p1, assign_p2, trains, method=method)

    # apply mutations after composed crossovers
...
# ---------------------
# GeneticAlgorithm with adaptive crossover selection (per-operator probs)
# ---------------------
class GeneticAlgorithm:
    def __init__(self, locomotives: Dict[int, Locomotive], trains: Dict[int, Train],
        ...
        # per-operator base probabilities for crossover (user-specified)
        ...
        # build effective per-operator base probs aligned with self.crossover_methods
        # if not provided for some operator — default to 0.5
    ...
    def _adapt_crossover_weights(self, current_best: float):
        """
        Адаптируем в��са операторов кроссовера на основе улучшения best:
    ...
    def run(self) -> Chromosome:
        ...
        for gen in range(self.generations):
            ...
            # record current crossover weights and base probs in stats (matching order of self.crossover_methods)
...
# ---------------------
# Streamlit UI (русский) с возможностью задавать per-operator probabilities
# ---------------------
